...
const colors = {
    white: "rgb(234, 225, 240)",
    black: "rgb(29, 28, 31)",
    blue: "rgb(65, 89, 203)",
    red: "rgb(246, 63, 76)",
    yellow: "rgb(253, 187, 39)",
    teal: "rgb(89, 167, 175)",
};
...
function draw_update_cell(cell) {
    ...
    if (
        cell.style.background === colors.red ||
        cell.style.background === colors.blue
    ) {
        return;
    }
    if (cell.style.background === colors.black) {
        maze[row][col] = 0;
        console.log(colors.white);
        cell.style.background = colors.white;
    } else if (cell.style.background === colors.white) {
        maze[row][col] = "#";
        cell.style.background = colors.black;
    }
}
...
function display_maze() {
    ...
    for (let i = 0; i < maze.length; i++) {
        ...
        for (let j = 0; j < maze[i].length; j++) {
            ...
            grid_cell.id = `${i}-${j}`;
            if (maze[i][j] === "S") {
                grid_cell.style.background = colors.blue;
            } else if (maze[i][j] === "E") {
                grid_cell.style.background = colors.red;
            } else {
                grid_cell.style.background = colors.white;
            }
...
function bfs(check_diags) {
    ...
    while (!q.empty()) {
        ...
        for (let d = 0; d < dirs; ++d) {
            ...
            if (
                ...
                q.push(new_pos);
                cell_animation_order.push(new CellAnimation(new_pos, colors.yellow));
            }
        }
    }
    ...
    while (!reconstruct_q.empty()) {
        ...
        cell_animation_order.push(new CellAnimation(going_to, colors.teal));
    }
