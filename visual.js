...
function bfs_update_cell(cell, color) {
	...
	const cell_id = document.getElementById(`${point.row}-${point.col}`);
...
function bfs() {
	...
	while (!q.empty()) {
		const pos = q.pop();
		if (pos.row === get_end_cell().row && pos.row === get_end_cell().col) {
			break;
		}
		bfs_update_cell(pos, "orange");
		for (let d = 0; d < 4; ++d) {
			...
			if (
				...
				q.push(new_pos);
				bfs_update_cell(new_pos, "purple");
			}
		}
	}
}
